<template>
    ...
    <ion-content class="ion-padding">
        <ion-item>
            <ion-label position="stacked">Le nom et la description prédéfinis.</ion-label>
        </ion-item>
        <ion-item>
        ...
        <ion-item v-for="u of userPAC" :key="u.id">
            <ion-checkbox label-placement="start" v-model="u.isSelected">{{ u.firstname }} {{ u.lastname }}</ion-checkbox>
        </ion-item>
        <!--ion-item>
...
<script>
import {IonContent,IonHeader,IonTitle,IonToolbar,IonPage,IonButtons,IonButton,IonItem,IonInput,modalController, IonSelect, IonTextarea, IonDatetime, IonSelectOption, IonLabel, IonCheckbox} from '@ionic/vue';
...
export default {
    ...
    components:{
        IonContent,IonHeader,IonTitle,IonToolbar,IonPage,IonButtons,IonButton,IonItem,IonInput,modalController, IonSelect, IonTextarea, IonDatetime, IonSelectOption, IonLabel, IonCheckbox
    },
    methods:{
        cancel(){
        ...
        async confirm(){
            ...
            let nom_event = "Demande"; //Changer par la ligne en com au dessus quand on aura dans la bdd un boolean sur "demande"
            ...
            let maxID = await fetch('https://sae-repihandi-api.onrender.com/activities/max')
            maxID = await maxID.json();

            for(let u of bufPAC){
                await this.participe(maxID[0].id,u.id);
            }
