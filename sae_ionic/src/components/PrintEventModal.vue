...
<ion-page class="main_page">
    ...
    <ion-content class="ion-padding tout">
      ...
      <hr> 
        ...
        <ion-item v-if="PACs.length != eventData.capacity">
          <ion-select
        ...
        <ion-button expand="block" class="custom-button" :strong="true" @click="participe" v-if="PACs.length != eventData.capacity">Participer</ion-button>
  ...
  methods: {
    ...
    */
    async refreshData(){
      ...
      let oui = new Date( this.eventData.start)
      let non = new Date(this.eventData.end)
      oui.setHours(oui.getHours() + 2)
      non.setHours(non.getHours() + 2)
