        ...
        <div class="ion-padding">
          ...
          <vue-cal ref="vuecal" small hide-view-selector active-view="day"
            ...
            <template #event="{ event }">
              <div :class="event.request === true ? 'demande-event' : 'normal-event'"><!-- Changer la condition par le boolean -->
          ...
          <ion-button expand="block" class="custom-button" @click="openAskModal">Demander un créneau</ion-button>
...
<script lang="ts">
import { IonContent, IonHeader, IonPage, IonTitle, IonToolbar, IonButton, IonIcon, IonDatetime, IonDatetimeButton, IonModal, modalController,IonItem, IonList, IonSelect, IonSelectOption, IonButtons } from '@ionic/vue';
import Modaleevent from '../components/Modaleevent.vue';
import ModaleeventAsk from '../components/ModaleeventAsk.vue';
import VueCal from 'vue-cal';
import { notifications, removeOutline,  addOutline, arrowForwardOutline, arrowBackOutline} from 'ionicons/icons';
import 'vue-cal/dist/vuecal.css';
import PrintEventModal from '@/components/PrintEventModal.vue';
import PrintAskEventModal from '@/components/PrintAskEventModal.vue';
...
export default {
    ...
    data(){
        return{
            ...
            less: arrowBackOutline,
            add: arrowForwardOutline,
            event: []
        }
    },
    components:{
      ...
      PrintEventModal, VueCal, Modaleevent, IonButtons, IonItem, IonList, IonSelect, IonSelectOption
    },
    mounted(){
 ...
 */
      ...
      */
      async openModal(){
        ...
        /*this.$router.push({
            name: 'ModalEvent', //use name for router push
          })*/

      },
      async openAskModal(){
        const modal = await modalController.create({
          component: ModaleeventAsk,
          componentProps: { 
            activities: this.event,
            bar: this.selectedDate
          }
      ...
      async onEventClick(event){
        if (event.request != true){ //Changer la condition du if pour "if demande = 0" (quand ça sera dans la bdd), sert à différencier une activité et une demande d'activité
          const modal = await modalController.create({
          ...
          const modal = await modalController.create({
            component: PrintAskEventModal,
            componentProps: {
              eventData: event, // Passer l'événement au composant PrintEventAskModal
            }
          });
          modal.present();
          const { data, role } = await modal.onWillDismiss();
    ...
    *///
    managedate(eventliste){
      for(const event of eventliste){
        ...
        event.startTime = `${startDate.getHours()}:${(startDate.getMinutes() < 10 ? '0' : '') + startDate.getMinutes()}`;
        let endDate = new Date(event.end);
        event.endTime = `${endDate.getHours()}:${(endDate.getMinutes() < 10 ? '0' : '') + endDate.getMinutes()}`;
