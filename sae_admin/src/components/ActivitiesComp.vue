        ...
        <div class="data">
            <div class="title">
                ...
                <div class="listBtn">
                    ...
                    <div class="btn" @click="supprActivit">Supprimer</div>
                </div>
            </div>
            <div class="tab" v-if="displayMore">
                ...
                <div>
                    <h4>Capacité de l'activité : {{ data.capacity }}</h4>
                </div>
            </div> 
        </div>
    </div>
...
export default defineComponent({
    ...
    methods:{
        ...
        async supprActivit(){
            console.log("Suppression de l'activité :"+this.data.id)
            const sent = await fetch('https://sae-repihandi-api.onrender.com/activities/'+this.data.id, {method: 'DELETE'});
            // Déclenchez l'événement après la suppression
            this.$emit('activityDeleted', this.data.id);
        }
    }
})
</script>
